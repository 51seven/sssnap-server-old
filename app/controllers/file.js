...
exports.publicShow = function(req, res, next) {
  ...
  Upload.load(options).then(function(doc) {
    ...
    if(Signature !== key || (dateCompare > 60000 || dateCompare < -60000)) {
      return next(new status.Forbidden('Access invalid or expired', 'Either the link is expired or the signature in the link is wrong. A link is valid for 60 seconds.'));
    }
